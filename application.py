...
def submitPizza():
    state_info = str(request.form['state'].lower())
    print(state_info)
    if (state_info == "production"):
        print("this is productio")
        json_val = {
            "$class": "org.acme.biznet.Pizza",
            "PizzaId": "chicpizza",
        ...
        transactions_val = {
            "$class": "org.acme.biznet.ChangeStateTo" + state_info.title(),
            "pizza": "chicpizza"
        }
        ...
        rT = requests.post(
            'http://localhost:3000/api/ChangeStateTo' + state_info.title(),
            data=transactions_val)
    else:
        print("ths is else case")
        json_val = {
            "$class": "org.acme.biznet.Pizza",
            "pizzaId": "chicpizza",
...
def changeOwner(owner):
    ...
    # Change Ownership
    owner_data = {
        "$class": "org.acme.biznet.ChangeOwner",
        "pizza": "chicpizza",
        "newOwner": str(owner)
    }
    # POST Request 2: Issue a ChangeOwner transaction
    rT = requests.post('http://localhost:3000/api/ChangeOwner',
                       data=owner_data)
